...
public class RoleController {
	...
	public RoleService.RoleDto createRole(@Valid @RequestBody SaveRoleRequest request,
		...
		return roleService.createRole(TenantContext.getRequiredTenantId(), principal.id(), request.code(), request.name(), request.permissions());
	}
	...
	public RoleService.RoleDto updateRole(@PathVariable UUID id, @Valid @RequestBody UpdateRoleRequest request,
			@AuthenticationPrincipal UserPrincipal principal) {
		return roleService.updateRole(TenantContext.getRequiredTenantId(), principal.id(), id, request.name(), request.permissions());
	}

	@DeleteMapping("/roles/{id}")
	@Operation(summary = "Delete a custom role.")
	@PreAuthorize("@securityService.hasPermission('ADMINISTRACION_WRITE')")
	public void deleteRole(@PathVariable UUID id, @AuthenticationPrincipal UserPrincipal principal) {
		roleService.deleteRole(TenantContext.getRequiredTenantId(), principal.id(), id);
	}
